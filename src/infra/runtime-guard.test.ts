...
describe("runtime-guard", () => {
  it("parses semver with or without leading v", () => {
    ...
    expect(parseSemver("22.15.0-beta.1")).toEqual({ major: 22, minor: 15, patch: 0 });
    expect(parseSemver("invalid")).toBeNull();
  });

  it("compares versions correctly", () => {
    expect(isAtLeast({ major: 22, minor: 15, patch: 0 }, { major: 22, minor: 15, patch: 0 })).toBe(
      true,
    );
    expect(isAtLeast({ major: 22, minor: 16, patch: 0 }, { major: 22, minor: 15, patch: 0 })).toBe(
      true,
    );
    expect(isAtLeast({ major: 22, minor: 14, patch: 0 }, { major: 22, minor: 15, patch: 0 })).toBe(
      false,
    );
    expect(isAtLeast({ major: 21, minor: 9, patch: 0 }, { major: 22, minor: 15, patch: 0 })).toBe(
      false,
    );
  });
  ...
  it("validates runtime thresholds", () => {
    ...
    expect(isSupportedNodeVersion("22.15.0")).toBe(true);
    expect(isSupportedNodeVersion("22.14.9")).toBe(false);
